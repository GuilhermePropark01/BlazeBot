...
class Estrategia:
    ...
    def resultados(self, total_de_jogos):
        ...
        if self.data[0:4] == ['Vermelho', 'Preto', 'Preto', 'Preto']:
            msg = Telegram('Vermelho')
            retorno = msg.confirmar_vitoria(total_de_jogos)
            self.registro_jogos.append(self.data)
            ...
            msg = Telegram('Branco')
            retorno = msg.confirmar_vitoria_branco(total_de_jogos)
            self.registro_jogos.append(self.data)
        else:
            msg = Telegram('Vermelho')
            retorno = msg.confirmar_derrota(total_de_jogos)
            self.registro_jogos.append(self.data)

        return total_de_jogos